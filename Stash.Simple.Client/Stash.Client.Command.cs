...
namespace Stash.Simple.Client {
  ...
  public sealed class StashQuery {
    #region Constants
...
#pragma warning restore IDE0057 // Use range operator
    ...
    /// </summary>
    public StashQuery(StashConnection connection) {
    ...
    /// Connection
    /// </summary>
    public StashConnection Connection { get; }
    ...
    public async Task<(JsonDocument document, string error)> TryQueryAsync(string address, string query, HttpMethod method, CancellationToken token) {
      ...
      var response = await StashConnection.Client.SendAsync(req, token).ConfigureAwait(false);
